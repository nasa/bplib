...
void test_setup_allocation(BPLib_STOR_MEM_Pool_t *pool, BPLib_STOR_MEM_BlockContent_t *db, BPLib_STOR_MEM_BlockContent_t *apib)
{
    ...
    BPLib_STOR_MEM_SubqPushSingle(admin->free_blocks, &db->header.base_link);
    UT_SetHandlerFunction(UT_KEY(BPLib_STOR_MEM_RBT_SearchGeneric), UT_AltHandler_PointerReturn, api_content);
}
