...
static int rbtest_comparator(const BPLib_MEM_RBT_Link_t *link, void *arg)
{
    rbtest_node_t *node;
    int            ref_val;
...
void TestBplibMemCommon_RBT_Basics(void)
{
    ...
    UtAssert_VOIDCALL(BPLib_MEM_RBT_InitRoot(&tree));
    ...
    UtAssert_BOOL_TRUE(BPLib_MEM_RBT_NodeIsRed(&node_array[1].link));
    UtAssert_BOOL_TRUE(BPLib_MEM_RBT_NodeIsMember(&tree, &node_array[0].link));
...
static uint32 Test_RBT_DebugDisplaySubtree(uint32 node_print_depth, BPLib_MEM_RBT_Link_t *node)
{
    char           my_color;
...
static uint32 Test_RBT_CheckSubtree(BPLib_MEM_RBT_Link_t *node)
{
    uint32 my_black_height;
    ...
    if (BPLib_MEM_RBT_NodeIsRed(node))
    {
        ...
        UtAssert_BOOL_FALSE(BPLib_MEM_RBT_NodeIsRed(node->parent));
    }
    else
    {
        ++my_black_height;
 ...
 */
static void Test_RBT_CheckTree(BPLib_MEM_RBT_Root_t *tree)
{
...
void TestBplibMemCommon_RBT_Unique(void)
{
    ...
    for (i = 0; i <= 28; ++i)
    {
        ...
        {
            UtAssert_NULL(BPLib_MEM_RBT_SearchUnique(100 * i, &rbtree));
        }
    }
}
...
void TestBplibMemCommon_RBT_Iterator(void)
{
    ...
    {
        ++count;
        UtAssert_UINT32_GT(BPLib_MEM_RBT_GetKeyValue(it.position), last_val);
    ...
    {
        ++count;
        UtAssert_UINT32_LT(BPLib_MEM_RBT_GetKeyValue(it.position), last_val);
        last_val = BPLib_MEM_RBT_GetKeyValue(it.position);
        status   = BPLib_MEM_RBT_IterPrev(&it);
    }
    while (status == BP_SUCCESS);
