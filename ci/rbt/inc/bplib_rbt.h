...
/**
 * @brief Context object for use with iteration routines
 *
 * This object tracks the current position in the tree during
 * traversal/iteration operations
 *
 * Used with iterator routines: GotoMax, GotoMin, Next, Prev
 */
typedef struct BPLib_RBT_Iter
{
    const BPLib_RBT_Link_t *position; /**< Position in the tree / current node */
} BPLib_RBT_Iter_t;
 ...
 * @retval NULL if no matching node was found
 */
static inline BPLib_RBT_Link_t *BPLib_RBT_SearchUnique(BPLib_Val_t search_key_value, const BPLib_RBT_Root_t *tree)
{
