        ...
        for (i = 0; i < BP_MAX_LOCKS; i++)
        {
            if (locks[i] == NULL)
            {
                ...
                if (locks[i])
                {
                    ...
                    ret = pthread_mutex_init(&locks[i]->mutex, &mattr);
                    if (ret != 0)
                    {
                        fprintf(stderr, "pthread_mutex_init(): %s\n", strerror(ret));
                    }
                    ret = pthread_cond_init(&locks[i]->cond, &cattr);
                    if (ret != 0)
                    {
 ...
 *-------------------------------------------------------------------------------------*/
int bplib_os_wait_until_ms(bp_handle_t h, bplib_monotime_t abs_time)
{
    bplib_os_lock_t *lock = locks[bp_handle_to_serial(h, BPLIB_HANDLE_OS_BASE)];
    ...
    {
        until_time.tv_sec  = (BPLIB_TIME_TO_INT(abs_time) / 1000);
        until_time.tv_nsec = (BPLIB_TIME_TO_INT(abs_time) % 1000) * 1000000;
