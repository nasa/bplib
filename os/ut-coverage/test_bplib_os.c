...
void test_bplib_os_wait_until_ms(void)
{
     ...
     */
    bp_handle_t h = bp_handle_from_serial(100, BPLIB_HANDLE_OS_BASE);
    BPLib_TIME_MonotonicTime_t tm = BPLIB_TIME_FROM_INT(500000000);

    UtAssert_VOIDCALL(bplib_os_wait_until_ms(h, tm));
}
...
void test_bplib_os_get_dtntime_ms(void)
{
    ...
    UtAssert_EQ(uint64_t, bplib_os_get_dtntime_raw(), 53315200496);
}
