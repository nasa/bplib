...
int  rb_tree_insert(bp_val_t   value,
                    rb_tree_t *tree); /* Inserts number into a red black tree. Duplicates will not be inserted. */
int  rb_tree_delete(bp_val_t   value,
                    rb_tree_t *tree);  /* Deletes a number from a rb_tree_t and may lead to split nodes. */
int  rb_tree_destroy(rb_tree_t *tree); /* Frees all memory allocated for a rb_tree and recursively frees its nodes. */
int  rb_tree_goto_first(
     rb_tree_t *tree); /* Gets the node of lowest cid in the tree to serve as an iterator to calls of get next. */
int rb_tree_get_next(
