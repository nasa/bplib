...
BP_LOCAL_SCOPE int overwrite_node(rh_hash_t *rh_hash, bp_index_t index, bp_active_bundle_t bundle, bool overwrite)
{
    if (overwrite)
    {
        ...
        if (newest_index != NULL_INDEX)
            rh_hash->table[newest_index].after = index;
        if (oldest_index == NULL_INDEX)
...
int rh_hash_add(rh_hash_t *rh_hash, bp_active_bundle_t bundle, bool overwrite)
{
    ...
    else /* collision */
    {
        ...
        while ((rh_hash->table[open_index].bundle.sid != BP_SID_VACANT) && (open_index != curr_index))
        {
