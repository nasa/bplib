...
static int sockkeepalive(int socket_fd, int idle, int cnt, int intvl)
{
    ...
    optval = idle; // set TCP_KEEPIDLE on a socket to number of seconds the connection is idle before keepalive probes
                   // are sent
...
int sockcreate(int type, const char *ip_addr, int port, int is_server, int *block)
{
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next)
    {
        ...
        status = getnameinfo(rp->ai_addr, rp->ai_addrlen, host, SOCK_HOST_STR_LEN, serv, SOCK_SERV_STR_LEN,
