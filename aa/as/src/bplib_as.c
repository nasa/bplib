...
BPLib_Status_t BPLib_AS_Set(int16_t SourceEid, BPLib_AS_Counter_t Counter, uint32_t Value)
{
    ...
    /* Verify that the counter is valid */
    if ((int64_t) Value > 0xFFFFFFFF || (int64_t) Value < 0)
    { /* Value is out of bounds for uint32_t */
        Status = BPLIB_AS_SET_VALUE_BOUNDS_ERR;

        BPLib_EM_SendEvent(BPLIB_AS_SET_VALUE_BOUNDS_ERR_EID,
