...
void test_BPLib_STOR_QM_MaintenanceRequestWait(void)
{
    ...
    memset(&rtbl, 0, sizeof(BPLib_STOR_QM_QueueTbl_t));
    rtbl.last_intf_poll = BPLIB_MONOTIME_ZERO;

    UT_SetHandlerFunction(UT_KEY(BPLib_STOR_CACHE_Os_getMonotimeRaw), V7__routing_GetTime_Handler, NULL);
...
void test_BPLib_STOR_QM_PeriodicMaintenance(void)
{
    ...
    UT_SetHandlerFunction(UT_KEY(BPLib_STOR_CACHE_SampleMonotimeMs), V7__routing_GetTime_Handler, NULL);
