...
static int BPLib_STOR_PS_FileOffloadConfigure(BPLib_STOR_CACHE_Block_t *svc, int key, BPLib_STOR_QM_ModuleValtype_t vt,
                                        const void *val);
static int BPLib_STOR_PS_FileOffloadQuery(BPLib_STOR_CACHE_Block_t *svc, int key, BPLib_STOR_QM_ModuleValtype_t vt,
                                    const void **val);
...
static const BPLib_STOR_PS_OffloadApi_t BPLIB_FILE_OFFLOAD_INTERNAL_API = {
    .std.module_type = BPLib_STOR_QM_ModuleTypeOffload,
...
const BPLib_STOR_QM_ModuleApi_t *BPLIB_FILE_OFFLOAD_API =
    (const BPLib_STOR_QM_ModuleApi_t *)&BPLIB_FILE_OFFLOAD_INTERNAL_API;
...
int BPLib_STOR_PS_FileOffloadConfigure(BPLib_STOR_CACHE_Block_t *svc, int key, BPLib_STOR_QM_ModuleValtype_t vt, const void *val)
{
    ...
    if (state != NULL)
    {
        switch (key)
        {
            case BPLib_STOR_QM_ConfkeyOffloadBaseDir:
...
BPLib_Handle_t BPLib_STOR_CACHE_CreateFileStorage(BPLib_STOR_CACHE_Block_t *rtbl, const BPLib_IpnAddr_t *storage_addr)
{
    ...
    if (BPLib_HandleIsValid(intf_id))
    {
        ...
        if (BPLib_HandleIsValid(svc_id))
        {
            ...
            BPLib_STOR_QM_Configure(rtbl, intf_id, BPLib_STOR_QM_ConfkeyOffloadBaseDir,
                                       BPLib_STOR_QM_ModuleValtypeString, storage_path);
            BPLib_STOR_QM_Start(rtbl, intf_id);
        }
    }
