...
// TODO BPLib_STOR_CACHE_CalculateExpireTime is a candidate for the TIME module.
BPLib_TIME_MonotonicTime_t BPLib_STOR_CACHE_CalculateExpireTime(uint64_t create_time, BPLib_TIME_MonotonicTime_t ingress_time, uint64_t lifetime)
{
    BPLib_TIME_MonotonicTime_t ref;

    // TODO Fix invalid conversion of monotonic time given the caller passes create_time and lifetime as DTN times.
    ref.Time = create_time;
    if (!BPLIB_TIME_IS_VALID(ref))
    {
        ref = ingress_time;
    }

    ref.Time = ref.Time + lifetime; 
    return ref;
}
                                               ...
                                               BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    custody_info->this_cblk = BPLib_STOR_CACHE_BblockCanonicalAlloc(BPLib_STOR_CACHE_ParentPool(state), 0, NULL);
    if (custody_info->this_cblk != NULL)
    {
        ...
        {
            custody_block->canonical_logical_data.canonical_block.blockType = BPLib_STOR_CACHE_BlocktypeCustodytrackingblock;
            custody_block->canonical_logical_data.canonical_block.blockNum  = BPLib_STOR_CACHE_BlocktypeCustodytrackingblock;
...
int BPLib_STOR_CACHE_CustodyFindDacsMatch(const BPLib_RBT_Link_t *node, void *arg)
{
    ...
    store_entry = BPLib_STOR_CACHE_EntryFromLink(node, hash_rbt_link);

    /* everything in here should be a DACS, assert on it for now */
    assert(store_entry->state == BPLib_STOR_CACHE_EntryStateGenerateDacs);
...
bool BPLib_STOR_CACHE_CustodyFindPendingDacs(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_CustodianInfo_t *dacs_info)
{
    ...
    hash = BPLib_CRC_InitialValue(BPLIB_CACHE_CUSTODY_HASH_ALGORITHM);
    ...
    if (custody_rbt_link != NULL)
    {
        ...
        dacs_info->store_entry = BPLib_STOR_CACHE_EntryFromLink(custody_rbt_link, hash_rbt_link);
    }
...
{
    BPLib_STOR_CACHE_BblockCanonical_t *custody_block;
    BPLib_IpnAddr_t                   final_dest_addr;
    ...
    custody_info->prev_cblk = BPLib_STOR_CACHE_BblockPrimaryLocateCanonical(pri_block, BPLib_STOR_CACHE_BlocktypeCustodytrackingblock);
    if (custody_info->prev_cblk != NULL)
    {
        ...
        if (custody_block != NULL)
        {
            ...
            custody_block->canonical_logical_data.canonical_block.blockType = BPLib_STOR_CACHE_BlocktypePreviouscustodianblock;
        }
    }
}
                                                  ...
                                                  BPLIB_CT_AcceptPayloadBlock_t **pay_out)
{
    ...
    pblk   = NULL;
    status = BPLIB_ERROR;
    ppool  = BPLib_STOR_CACHE_ParentPool(state);

    do
    {
        pblk      = BPLib_STOR_CACHE_BblockPrimaryAlloc(ppool, 0, NULL, BPLIB_MPOOL_ALLOC_PRI_MED, BPLIB_MONOTIME_ZERO);
        ...
        pay->canonical_block.blockNum  = BPLib_STOR_CACHE_BlocktypePayloadblock;
        pay->canonical_block.blockType = BPLib_STOR_CACHE_BlocktypeCustodyacceptpayloadblock;
        pay->canonical_block.crctype   = BPLib_CRC_Type_CRC16;
    ...
    if (pblk != NULL && status != BPLIB_SUCCESS)
    {
...
void BPLib_STOR_CACHE_CustodyOpenDacs(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    BPLib_STOR_CACHE_BblockPrimary_t      *pri_block;
    BPLIB_CT_AcceptPayloadBlock_t *ack_content;
    ...
    if (store_entry != NULL && pending_bundle != NULL)
    {
        ...
        self_intf_id                                 = BPLib_STOR_CACHE_GetExternalId(BPLib_STOR_CACHE_StateSeldblock(state));
        pri_block->data.delivery.delivery_policy     = BPLib_STOR_CACHE_PolicyDeliveryLocalAck;
        pri_block->data.delivery.local_retx_interval = BPLIB_CACHE_FAST_RETRY_TIME;
        pri_block->data.delivery.ingress_intf_id     = self_intf_id;
        pri_block->data.delivery.storage_intf_id     = self_intf_id;
        pri_block->data.delivery.committed_storage_id = (BPLib_STOR_CACHE_Sid_t)sblk;
        ...
        store_entry->action_time   = BPLib_STOR_CACHE_TimeAddMs(pri_block->data.delivery.ingress_time, BPLIB_CACHE_DACS_OPEN_TIME);
        store_entry->expire_time   = BPLib_STOR_CACHE_TimeAddMs(pri_block->data.delivery.ingress_time, BPLIB_CACHE_DACS_LIFETIME);
        store_entry->duct_id_copy  = state->self_addr;
        store_entry->duct_seq_copy = pri_block->data.logical.creationTimeStamp.sequence_num;
...
{
    BPLIB_CT_AcceptPayloadBlock_t *payload;
    uint64_t                       i;
    ...
    if (custody_info->store_entry != NULL)
    {
        ...
        if (i == payload->num_entries && i < BPLIB_DACS_MAX_SEQ_PER_PAYLOAD)
        {
        ...
        if (payload->num_entries == BPLIB_DACS_MAX_SEQ_PER_PAYLOAD)
        {
            BPLib_STOR_CACHE_CustodyFinalizeDacs(state, custody_info->store_entry);
            BPLib_STOR_CACHE_EntryMakePending(custody_info->store_entry, 0, BPLIB_STORE_FLAG_ACTION_TIME_WAIT);
        }
    }
}
                                            ...
                                            const BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    if (custody_info->prev_cblk != NULL)
    {
        ...
        dacs_info.duct_id         = custody_info->duct_id;
        dacs_info.sequence_num    = custody_info->sequence_num;
        dacs_info.final_dest_node = custody_info->final_dest_node;

        if (!BPLib_STOR_CACHE_CustodyFindPendingDacs(state, &dacs_info))
        {
            /* open DACS bundle did not exist - make an empty one now */
            BPLib_STOR_CACHE_CustodyOpenDacs(state, &dacs_info);
        }

        BPLib_STOR_CACHE_CustodyAppendDacs(state, &dacs_info);
    }
}
                                        ...
                                        BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    else
    {
        ...
        BPLib_STOR_CACHE_CustodyInsertTrackingBlock(state, pri_block, custody_info);
    }
    ...
    if (custody_info->this_cblk != NULL)
    {
        ...
        BPLib_STOR_CACHE_CustodyUpdateTrackingBlock(state, custody_info);
    }
}
...
int BPLib_STOR_CACHE_CustodyFindBundleMatch(const BPLib_RBT_Link_t *node, void *arg)
{
    ...
    result = compare_numeric(custody_info->sequence_num, store_entry->duct_seq_copy);
    if (result == 0)
    {
...
bool BPLib_STOR_CACHE_CustodyFindExistingBundle(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    if (custody_rbt_link != NULL)
    {
        custody_info->store_entry = BPLib_STOR_CACHE_EntryFromLink(custody_rbt_link, hash_rbt_link);
...
bool BPLib_STOR_CACHE_CustodyCheckDacs(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_Block_t *qblk)
{
    ...
    if (pri_block != NULL && pri_block->data.logical.controlFlags.isAdminRecord)
    {
        ...
        c_block = BPLib_STOR_CACHE_BblockCanonicalCast(
            BPLib_STOR_CACHE_BblockPrimaryLocateCanonical(pri_block, BPLib_STOR_CACHE_BlocktypeCustodyacceptpayloadblock));
        if (c_block != NULL)
        {
            /* it is an acceptance block (dacs) */
            BPLib_STOR_CACHE_CustodyProcessRemoteDacsBundle(
...
void BPLib_STOR_CACHE_CustodyStoreBundle(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_Block_t *qblk)
{
    ...
    BPLib_STOR_CACHE_CustodyInitInfoFromPblock(&custody_info, pri_block);
    ...
    if (custody_info.store_entry != NULL)
    {
        ...
        custody_info.store_entry->state  = BPLib_STOR_CACHE_EntryStateIdle;
        ...
        custody_info.store_entry->expire_time =
            ...
            pri_block->data.delivery.ingress_time, pri_block->data.logical.lifetime);

        pri_block->data.delivery.storage_intf_id = BPLib_STOR_CACHE_GetExternalId(BPLib_STOR_CACHE_StateSeldblock(state));
        ...
        BPLib_STOR_CACHE_FsmExecute(sblk);
    }
    else if (sblk != NULL)
    {
        /*
