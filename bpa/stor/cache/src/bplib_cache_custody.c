                                               ...
                                               BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    custody_info->this_cblk = BPLib_STOR_CACHE_BblockCanonicalAlloc(BPLib_STOR_CACHE_Module_parent_pool(state), 0, NULL);
...
int BPLib_STOR_CACHE_Module_custody_find_dacs_match(const BPLib_STOR_CACHE_RBT_Link_t *node, void *arg)
{
    ...
    BPLib_STOR_CACHE_Module_dacs_pending_t   *dacs_pending;
    BPLib_STOR_CACHE_Module_custodian_info_t *custody_info;
    int                           result;

    custody_info = arg;

    store_entry = BPLib_STOR_CACHE_Module_entry_from_link(node, hash_rbt_link);

    /* everything in here should be a DACS, assert on it for now */
    assert(store_entry->state == BPLib_STOR_CACHE_Module_entry_state_generate_dacs);
    ...
    result = BPLib_STOR_CACHE_CustodyCompareIpn2ipn(&custody_info->custodian_id, &dacs_pending->prev_custodian_id);

    if (result == 0)
    {
        /* confirm match of the flow ID */
        result = BPLib_STOR_CACHE_CustodyCompareIpn2eid(&custody_info->flow_id, &dacs_pending->payload_ref->flow_source_eid);
    }
    return result;
    #else // STOR
    return 0;
...
bool BPLib_STOR_CACHE_Module_custody_find_pending_dacs(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_custodian_info_t *dacs_info)
{
    ...
    custody_rbt_link = BPLib_STOR_CACHE_RBT_SearchGeneric(dacs_info->eid_hash, &state->dacs_index,
                                                BPLib_STOR_CACHE_Module_custody_find_dacs_match, dacs_info);
    if (custody_rbt_link != NULL)
    {
        /* its a complete match */
        dacs_info->store_entry = BPLib_STOR_CACHE_Module_entry_from_link(custody_rbt_link, hash_rbt_link);
    }
                                               ...
                                               BPLib_STOR_CACHE_BblockPrimary_t *pri_block)
{
    ...
    BPLib_STOR_CACHE_CustodyGetEid(&custody_info->flow_id, &pri_block->data.logical.sourceEID);
    custody_info->sequence_num = pri_block->data.logical.creationTimeStamp.sequence_num;

    BPLib_STOR_CACHE_CustodyGetEid(&final_dest_addr, &pri_block->data.logical.destinationEID);
    ...
    if (custody_info->prev_cblk != NULL)
    {
        ...
        if (custody_block != NULL)
        {
            ...
            BPLib_STOR_CACHE_CustodyGetEid(&custody_info->custodian_id,
                                                  ...
                                                  bp_custody_accept_payload_block_t **pay_out)
{
    ...
    pblk   = NULL;
    status = BP_ERROR;
    ppool  = BPLib_STOR_CACHE_Module_parent_pool(state);
    ...
    do
    {
        ...
        BPLib_STOR_CACHE_CustodySetEid(&pri->sourceEID, &state->self_addr);
        BPLib_STOR_CACHE_CustodySetEid(&pri->reportEID, &state->self_addr);
...
void BPLib_STOR_CACHE_Module_custody_open_dacs(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    BPLib_STOR_CACHE_Module_entry_t               *store_entry;
    BPLib_STOR_CACHE_Module_dacs_pending_t        *dacs_pending;
    BPLib_STOR_CACHE_Ref_t                  pending_bundle;
    BPLib_STOR_CACHE_BblockPrimary_t      *pri_block;
    ...
    sblk           = BPLib_STOR_CACHE_GenericDataAlloc(BPLib_STOR_CACHE_Module_parent_pool(state), BPLIB_STORE_SIGNATURE_ENTRY, state);
    pending_bundle = BPLib_STOR_CACHE_Module_custody_create_dacs(state, &pri_block, &ack_content);
    ...
    if (store_entry != NULL && pending_bundle != NULL)
    {
        ...
        self_intf_id                                 = BPLib_STOR_CACHE_GetExternalId(BPLib_STOR_CACHE_Module_state_self_block(state));
        ...
        BPLib_STOR_CACHE_CustodySetEid(&pri_block->data.logical.destinationEID, &custody_info->custodian_id);
        BPLib_STOR_CACHE_CustodySetEid(&ack_content->flow_source_eid, &custody_info->flow_id);
        ...
        BPLib_STOR_CACHE_CustodyGetEid(&dacs_pending->prev_custodian_id, &pri_block->data.logical.destinationEID);

        BPLib_STOR_CACHE_RBT_InsertValueGeneric(custody_info->eid_hash, &state->dacs_index, &store_entry->hash_rbt_link,
                                       BPLib_STOR_CACHE_Module_custody_find_dacs_match, custody_info);
        BPLib_STOR_CACHE_Module_entry_make_pending(
...
void BPLib_STOR_CACHE_Module_custody_append_dacs(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    if (custody_info->store_entry != NULL)
    {
        ...
        {
            BPLib_STOR_CACHE_Module_custody_finalize_dacs(state, custody_info->store_entry);
            BPLib_STOR_CACHE_Module_entry_make_pending(custody_info->store_entry, 0, BPLIB_STORE_FLAG_ACTION_TIME_WAIT);
        }
    }
}
                                            ...
                                            const BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    if (custody_info->prev_cblk != NULL)
    {
        ...
        if (custody_block != NULL)
        {
            BPLib_STOR_CACHE_CustodyGetEid(&dacs_info.custodian_id,
                       &custody_block->canonical_logical_data.data.custody_tracking_block.current_custodian);
        }
        ...
        if (!BPLib_STOR_CACHE_Module_custody_find_pending_dacs(state, &dacs_info))
        {
            /* open DACS bundle did not exist - make an empty one now */
            BPLib_STOR_CACHE_Module_custody_open_dacs(state, &dacs_info);
        }

        BPLib_STOR_CACHE_Module_custody_append_dacs(state, &dacs_info);
    }
    #endif // STOR
}
...
void BPLib_STOR_CACHE_Module_custody_update_tracking_block(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    if (custody_block != NULL)
    {
        BPLib_STOR_CACHE_CustodySetEid(&custody_block->canonical_logical_data.data.custody_tracking_block.current_custodian,
                   &state->self_addr);
                                        ...
                                        BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    ...
    else
    {
        ...
        BPLib_STOR_CACHE_Module_custody_insert_tracking_block(state, pri_block, custody_info);
    }
    ...
    if (custody_info->this_cblk != NULL)
    {
        ...
        BPLib_STOR_CACHE_Module_custody_update_tracking_block(state, custody_info);
    }
}
...
int BPLib_STOR_CACHE_Module_custody_find_bundle_match(const BPLib_STOR_CACHE_RBT_Link_t *node, void *arg)
{
    ...
    result = BPLib_STOR_CACHE_CustodyCompareNumeric(custody_info->sequence_num, store_entry->flow_seq_copy);
    if (result == 0)
    {
        result = BPLib_STOR_CACHE_CustodyCompareIpn2ipn(&custody_info->flow_id, &store_entry->flow_id_copy);
    }
...
bool BPLib_STOR_CACHE_Module_custody_find_existing_bundle(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_custodian_info_t *custody_info)
{
    BPLib_STOR_CACHE_RBT_Link_t *custody_rbt_link;
    bp_crcval_t       hash;
    ...
    custody_rbt_link = BPLib_STOR_CACHE_RBT_SearchGeneric(custody_info->eid_hash, &state->bundle_index,
                                                BPLib_STOR_CACHE_Module_custody_find_bundle_match, custody_info);
    if (custody_rbt_link != NULL)
    {
        custody_info->store_entry = BPLib_STOR_CACHE_Module_entry_from_link(custody_rbt_link, hash_rbt_link);
                                                    ...
                                                    const bp_custody_accept_payload_block_t *ack_payload)
{
    ...
    BPLib_STOR_CACHE_CustodyGetEid(&custody_info.custodian_id, &pri_block->data.logical.destinationEID);
    BPLib_STOR_CACHE_CustodyGetEid(&custody_info.flow_id, &ack_payload->flow_source_eid);

    for (i = 0; i < ack_payload->num_entries; ++i)
    {
        custody_info.sequence_num = ack_payload->sequence_nums[i];
        if (BPLib_STOR_CACHE_Module_custody_find_existing_bundle(state, &custody_info))
        {
            /* found it ! */
            ...
            BPLib_STOR_CACHE_Module_entry_make_pending(custody_info.store_entry, 0, BPLIB_STORE_FLAG_LOCAL_CUSTODY);
        }
    }
    #endif // STOR
}
...
void BPLib_STOR_CACHE_Module_custody_finalize_dacs(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Module_entry_t *store_entry)
{
    ...
    if (BPLib_STOR_CACHE_RBT_NodeIsMember(&state->dacs_index, &store_entry->hash_rbt_link))
    {
        BPLib_STOR_CACHE_RBT_ExtractNode(&state->dacs_index, &store_entry->hash_rbt_link);
    }
}
...
bool BPLib_STOR_CACHE_Module_custody_check_dacs(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Block_t *qblk)
{
    ...
    if (pri_block != NULL && pri_block->data.logical.controlFlags.isAdminRecord)
    {
        ...
        if (c_block != NULL)
        {
            ...
            BPLib_STOR_CACHE_Module_custody_process_remote_dacs_bundle(
...
void BPLib_STOR_CACHE_Module_custody_store_bundle(BPLib_STOR_CACHE_Module_state_t *state, BPLib_STOR_CACHE_Block_t *qblk)
{
    ...
    BPLib_STOR_CACHE_Module_custody_init_info_from_pblock(&custody_info, pri_block);
    ...
    if (BPLib_STOR_CACHE_Module_custody_find_existing_bundle(state, &custody_info))
    {
        ...
        BPLib_STOR_CACHE_Module_custody_ack_tracking_block(state, &custody_info);
        return;
    }
    ...
    if (custody_info.store_entry != NULL)
    {
        ...
        custody_info.store_entry->state  = BPLib_STOR_CACHE_Module_entry_state_idle;
        ...
        BPLib_STOR_CACHE_RBT_InsertValueGeneric(custody_info.final_dest_node, &state->dest_eid_jphfix_index,
        ...
        BPLib_STOR_CACHE_RBT_InsertValueGeneric(custody_info.eid_hash, &state->bundle_index,
                                       &custody_info.store_entry->hash_rbt_link, BPLib_STOR_CACHE_Module_custody_find_bundle_match,
                                       &custody_info);
        ...
        BPLib_STOR_CACHE_CustodyGetEid(&custody_info.store_entry->flow_id_copy, &pri_block->data.logical.sourceEID);
        custody_info.store_entry->expire_time =
            pri_block->data.logical.creationTimeStamp.time + pri_block->data.logical.lifetime;

        pri_block->data.delivery.storage_intf_id = BPLib_STOR_CACHE_GetExternalId(BPLib_STOR_CACHE_Module_state_self_block(state));
        ...
        else
        {
            ...
            {
                BPLib_STOR_CACHE_Module_custody_process_bundle(state, pri_block, &custody_info);
            }
        ...
        BPLib_STOR_CACHE_Module_fsm_execute(sblk);
    }
    else if (sblk != NULL)
    {
        /*
