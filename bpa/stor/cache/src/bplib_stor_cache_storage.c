    ...
    if (bp_handle_is_valid(intf_id))
    {
        ...
        if (bp_handle_is_valid(svc_id))
        {
            ...
            BPLib_STOR_CACHE_Configure(rtbl, intf_id, BPLib_STOR_CACHE_ConfkeyOffloadBaseDir,
                                  BPLib_STOR_CACHE_ValtypeString, storage_path);
...
bp_handle_t BPLib_STOR_CACHE_CreateNodeIntf(BPLib_STOR_QM_QueueTbl_t *rtbl, bp_ipn_t node_num)
{
    ...
    if (!bp_handle_is_valid(intf_id))
    {
        bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "BPLib_STOR_QM_AddIntf failed\n");
        return BP_INVALID_HANDLE;
    }

    if (BPLib_STOR_QM_Add(rtbl, node_num, ~(bp_ipn_t)0, intf_id) < 0)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "BPLib_STOR_QM_Add failed\n");
    }
...
int BPLib_STOR_CACHE_Eid2ipn(const char *eid, size_t len, bp_ipn_t *node, bp_ipn_t *service)
{
    ...
    if (eid == NULL)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID is null\n");
    }

    /* Sanity Check Length of EID */
    if (len < 7)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID must be at least 7 characters, act: %d\n", len);
    }
    else if (len > BP_MAX_EID_STRING)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID cannot exceed %d bytes in length, act: %d\n", BP_MAX_EID_STRING,
                     len);
    }
    ...
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID (%s) must start with 'ipn:'\n", eid);
    }
    ...
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "Unable to find dotted notation in EID (%s)\n", eid);
    }
    ...
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "Unable to parse EID (%s) node number\n", eid);
    }
    ...
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "Unable to parse EID (%s) service number\n", eid);
    }
...
int BPLib_STOR_CACHE_Ipn2eid(char *eid, size_t len, bp_ipn_t node, bp_ipn_t service)
{
    ...
    if (eid == NULL)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID buffer is null\n");
    }

    /* Sanity Check Length of EID Buffer */
    if (len < 7)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID buffer must be at least 7 characters, act: %d\n", len);
    }
    else if (len > BP_MAX_EID_STRING)
    {
        return bplog(NULL, BPLIB_FLAG_API_ERROR, "EID buffer cannot exceed %d bytes in length, act: %d\n",
 ...
 *
 *-----------------------------------------------------------------*/
int BPLib_STOR_CACHE_QueryInteger(BPLib_STOR_QM_QueueTbl_t *rtbl, bp_handle_t intf_id,BPLib_STOR_CACHE_Variable_t var_id, bp_sval_t *value)
{
    int retval;
 ...
 *
 *-----------------------------------------------------------------*/
int BPLib_STOR_CACHE_ConfigInteger(BPLib_STOR_QM_QueueTbl_t *rtbl, bp_handle_t intf_id,BPLib_STOR_CACHE_Variable_t var_id, bp_sval_t value)
{
    int retval;
