...
#include "bplib_stor_cache_encode_internal.h"
                                               ...
                                               BPLib_STOR_CACHE_BblockPrimary_t *pri_block)
{
    ...
    v7_get_eid(&custody_info->duct_id, &pri_block->data.logical.sourceEID);
...
void BPLib_STOR_CACHE_CustodyOpenDacs(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_CustodianInfo_t *custody_info)
{
    ...
    if (store_entry != NULL && pending_bundle != NULL)
    {
        ...
        store_entry->action_time   =BPLib_STOR_CACHE_TimeAddMs(pri_block->data.delivery.ingress_time, BP_CACHE_DACS_OPEN_TIME);
        store_entry->expire_time   =BPLib_STOR_CACHE_TimeAddMs(pri_block->data.delivery.ingress_time, BP_CACHE_DACS_LIFETIME);
                                                    ...
                                                    const BPLIB_CT_AcceptPayloadBlock_t *ack_payload)
{
    ...
    v7_get_eid(&custody_info.duct_id, &ack_payload->duct_source_eid);
...
void BPLib_STOR_CACHE_CustodyStoreBundle(BPLib_STOR_CACHE_State_t *state, BPLib_STOR_CACHE_Block_t *qblk)
{
    ...
    if (custody_info.store_entry != NULL)
    {
        ...
        v7_get_eid(&custody_info.store_entry->duct_id_copy, &pri_block->data.logical.sourceEID);
        custody_info.store_entry->expire_time =BPLib_STOR_CACHE_CalculateExpireTime(pri_block->data.logical.creationTimeStamp.time,
