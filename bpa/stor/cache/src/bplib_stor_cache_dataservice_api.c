    ...
    const void *content, size_t size)
{
    ...
    do
    {
        ...
        if (pri_block == NULL)
        {
            bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "Failed to allocate primary block\n");
            break;
        }
        ...
        {
            bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "Failed encoding pri block\n");
            break;
        }
        ...
        if (ccb_pay == NULL)
        {
            bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "Failed to allocate payload block\n");
            break;
        }
        ...
        {
            bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "Failed encoding pay block\n");
            break;
        }
    ...
    void *content, size_t *size)
{
    ...
    do
    {
        ...
        if (pri == NULL)
        {
            ...
            bplog(NULL, BPLIB_FLAG_INCOMPLETE, "%s(): Not a primary block\n", __func__);
            break;
        }
        ...
        if (ccb_pay == NULL)
        {
            ...
            bplog(NULL, BPLIB_FLAG_INCOMPLETE, "%s(): No payload\n", __func__);
            break;
        }
        ...
        {
            /* No payload */
            bplog(NULL, BPLIB_FLAG_INCOMPLETE, "%s(): Incorrectly sized bundle\n", __func__);
            break;
        }
        ...
        {
            bplog(NULL, BPLIB_FLAG_INCOMPLETE, "%s(): Failed to copy complete payload\n", __func__);
            break;
        }
...
int BPLib_STOR_CACHE_ServiceductForwardIngress(void *arg, BPLib_STOR_CACHE_Block_t *subq_src)
{
    ...
    BPLib_STOR_CACHE_Duct_t             *storage_duct;
    int                             forward_count;
    ...
    if (base_intf == NULL)
    {
        return bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "intf_block invalid\n");
    }

    curr_duct = BPLib_STOR_CACHE_DuctCast(intf_block);
    if (curr_duct == NULL)
    {
        return -1;
    }
    ...
    while (true)
    {
        qblk = BPLib_STOR_CACHE_DuctTryPull(&curr_duct->ingress, BPLIB_MONOTIME_ZERO);
        if (qblk == NULL)
        {
            /* no more bundles */
            break;
        ...
        {
            storage_duct = BPLib_STOR_CACHE_DuctCast(BPLib_STOR_CACHE_Dereference(base_intf->storage_service));
            if (storage_duct != NULL && BPLib_STOR_CACHE_DuctTryPush(&storage_duct->egress, qblk, BPLIB_MONOTIME_ZERO))
            {
                qblk = NULL;
            }
        }
...
int BPLib_STOR_CACHE_ServiceductForwardEgress(void *arg, BPLib_STOR_CACHE_Block_t *subq_src)
{
    ...
    while (true)
    {
        pblk = BPLib_STOR_CACHE_DuctTryPull(&curr_duct->egress, BPLIB_MONOTIME_ZERO);
        if (pblk == NULL)
        {
        ...
        if (pri_block != NULL)
        {
            ...
            if (next_duct_ref != NULL)
            {
                next_duct = BPLib_STOR_CACHE_DuctCast(BPLib_STOR_CACHE_Dereference(next_duct_ref));
                if (next_duct != NULL && BPLib_STOR_CACHE_DuctTryPush(&next_duct->egress, pblk, BPLIB_MONOTIME_ZERO))
                {
                    pblk = NULL;
                }
            }
                                  ...
                                  BPLib_STOR_CACHE_Ref_t endpoint_intf_ref)
{
   ...
   BPLib_STOR_CACHE_ServiceEndpt_t          *endpoint_intf;
    ...
    if (base_intf != NULL)
    {
        ...
        if (temp_block == NULL)
        {
            bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): out of memory when binding socket\n", __func__);
            return BPLIB_ERROR;
        }
...
int BPLib_STOR_CACHE_DataserviceEventImpl(void *arg, BPLib_STOR_CACHE_Block_t *intf_block)
{
    ...
    if (event->event_type == BPLib_STOR_CACHE_DuctEventUp)
    {
        ...
        {
            BPLib_STOR_CACHE_DuctEnable(&duct->ingress, BP_MPOOL_SHORT_SUBQ_DEPTH);
        }
        else
        {
...
bp_handle_t BPLib_STOR_CACHE_DataserviceAddBaseIntf(BPLib_STOR_QM_QueueTbl_t *rtbl, bp_ipn_t node_number)
{
   ...
   BPLib_STOR_CACHE_DataserviceInit(pool);
    ...
    /* Allocate Blocks */
    sblk = BPLib_STOR_CACHE_DuctAlloc(pool, BPLIB_BLOCKTYPE_SERVICE_BASE, NULL);
    if (sblk == NULL)
    {
    ...
    if (base_intf == NULL)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "Failed to convert sblk to ref\n");
    ...
    BPLib_STOR_CACHE_Ref_t blkref)
{
    ...
    duct = BPLib_STOR_CACHE_DuctCast(BPLib_STOR_CACHE_Dereference(blkref));
    if (duct == NULL)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): bad descriptor\n", __func__);
        return BP_INVALID_HANDLE;
    }
    ...
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): no parent intf for node %lu\n", __func__,
    ...
    if (status != BPLIB_SUCCESS)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): cannot add service %lu to node %lu - duplicate?\n", __func__,
    ...
    else
    {
        ...
        if (!bp_handle_is_valid(self_intf_id))
        {
           BPLib_STOR_CACHE_ServiceductRemoveFromBase(BPLib_STOR_CACHE_Dereference(parent_block_ref), ipn->service_number);
            bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): could not register service %lu\n", __func__,
                  (unsigned long)ipn->node_number);
        }
...
BPLib_STOR_CACHE_Ref_t BPLib_STOR_CACHE_DataserviceDetach(BPLib_STOR_QM_QueueTbl_t *tbl, const bp_ipn_addr_t *ipn)
{
    ...
    if (refptr != NULL)
    {
        ...
        duct = BPLib_STOR_CACHE_DuctCast(BPLib_STOR_CACHE_Dereference(refptr));
        if (duct != NULL)
        {
...
bp_socket_t *BPLib_STOR_CACHE_CreateSocket(BPLib_STOR_QM_QueueTbl_t *rtbl)
{
   ...
   BPLib_STOR_CACHE_SocketInfo_t *sock;
    ...
    /* Allocate Duct */
    sblk = BPLib_STOR_CACHE_DuctAlloc(pool, BPLIB_BLOCKTYPE_SERVICE_SOCKET, NULL);
    if (sblk != NULL)
    {
    ...
    {
        bplog(NULL, BPLIB_FLAG_OUT_OF_MEMORY, "Cannot open socket: not enough memory\n");
        sock_ref = NULL;
    }
...
int BPLib_STOR_CACHE_BindSocket(bp_socket_t *desc, const bp_ipn_addr_t *source_ipn)
{
    ...
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): socket already connected\n", __func__);
        return BPLIB_ERROR;
    }

    if (source_ipn->node_number == 0)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): invalid source node number\n", __func__);
        return BPLIB_ERROR;
    }
...
int BPLib_STOR_CACHE_ConnectSocket(bp_socket_t *desc, const bp_ipn_addr_t *destination_ipn)
{
   BPLib_STOR_CACHE_SocketInfo_t *sock;
    ...
    {
        /* not yet bound */
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): socket not bound\n", __func__);
        return -1;
    }
    ...
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): invalid destination node number\n", __func__);
        return -1;
    }
...
int BPLib_STOR_CACHE_Send(bp_socket_t *desc, const void *payload, size_t size, uint32_t timeout)
{
   ...
   BPLib_STOR_CACHE_SocketInfo_t          *sock;
    ...
    sock_ref      = (BPLib_STOR_CACHE_Ref_t)desc;
    ingress_time.Time  = BPLib_STOR_CACHE_GetMonotonicTime();
    ...
    duct = BPLib_STOR_CACHE_DuctCast(BPLib_STOR_CACHE_Dereference(sock_ref));
    if (duct == NULL)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): bad duct ref - is socket connected?\n", __func__);
        return BPLIB_ERROR;
    }
    ...
    if (pblk == NULL)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): unable to alloc pri block\n", __func__);
    ...
    if (status != BPLIB_SUCCESS)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): cannot bundleize payload, out of memory?\n", __func__);
        return status;
    }
    ...
    if (refptr == NULL)
    {
        /* not expected... */
        BPLib_STOR_CACHE_RecycleBlock(pblk);
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "Cannot convert payload to ref\n");
        return BPLIB_ERROR;
    }
    ...
    if (rblk != NULL)
    {
        ...
        if (pri_block != NULL)
        {
            ...
            pri_block->data.delivery.ingress_time    = ingress_time.Time;
        }

        if (BPLib_STOR_CACHE_DuctTryPush(&duct->ingress, rblk, ingress_limit))
        {
    ...
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): failed to create block ref, out of memory?\n", __func__);
        status = BPLIB_ERROR;
    }
...
int BPLib_STOR_CACHE_Recv(bp_socket_t *desc, void *payload, size_t *size, uint32_t timeout)
{
   ...
   BPLib_STOR_CACHE_SocketInfo_t          *sock;
    ...
    if (duct == NULL)
    {
        bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): bad duct_ref\n", __func__);
        return BPLIB_ERROR;
    }
    ...
    pblk = BPLib_STOR_CACHE_DuctTryPull(&duct->egress, egress_time_limit);
    ...
    else
    {
        ...
        {
            bplog(NULL, BPLIB_FLAG_DIAGNOSTIC, "%s(): unable to create ref from pblk\n", __func__);
            status = BPLIB_ERROR;
        }
        ...
        if (status == BPLIB_SUCCESS)
        {
            ...
            if (pri_block != NULL)
            {
                ...
                pri_block->data.delivery.egress_time = BPLib_STOR_CACHE_GetDtnTime();
            }
        }
