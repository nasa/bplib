...
void test_bplib_route_ingress_to_parent(void)
{
    ...
    subq_src.next          = calloc(1, sizeof(BPLib_MEM_block_t));
...
void test_bplib_route_register_generic_intf(void)
{
    ...
    memset(&flow_block, 0, sizeof(BPLib_MEM_block_t));
...
void test_bplib_route_del_intf(void)
{
    ...
    memset(&ifp, 0, sizeof(BPLib_MEM_flow_t));
...
void test_bplib_route_push_ingress_bundle(void)
{
    ...
    bp_handle_t          intf_id;
    BPLib_MEM_block_t *cb = NULL;
    BPLib_MEM_flow_t   flow_block;

    memset(&tbl, 0, sizeof(bplib_routetbl_t));
    memset(&intf_id, 0, sizeof(bp_handle_t));
    memset(&flow_block, 0, sizeof(BPLib_MEM_flow_t));
...
void test_bplib_route_add(void)
{
    ...
    memset(&pool, 0, sizeof(BPLib_MEM_t));
    rtbl.pool = &pool;

    UT_SetHandlerFunction(UT_KEY(bplib_os_calloc), UT_lib_AltHandler_PointerReturn, &rtbl);
    UT_SetHandlerFunction(UT_KEY(BPLib_MEM_create), UT_lib_AltHandler_PointerReturn, &pool);
    memset(&intf_id, 0, sizeof(bp_handle_t));
    ...
    UT_SetHandlerFunction(UT_KEY(BPLib_MEM_create), UT_lib_AltHandler_PointerReturn, NULL);
}
